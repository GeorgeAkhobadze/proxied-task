...
- If a cart item quantity is **UPDATED** through the `updateItemQuantity` mutation the cart's `hash` will _NOT_ be updated.
...
**_Important: When a cart item changes, prompt the user to acknowledge the new cart state. If the user does not acknowledge, they should not be allowed to proceed to checkout._**
